        ...
        <div class = "d-flex shadow-sm p-3 mb-5 bg-body rounded" style="width: auto;">
            ...
            <form class = "p-3" method="POST" action="{{ route('user.update', Auth::user()->id) }}">
                ...
                <div class="mb-3" style="width: 350px;">
                    ...
                    @else
                    <input type="date" class="form-control" id="verified" value = "{{ Auth::user()->email_verified_at->format('Y-m-d') }}">
                    @endif
                </div>
                <div class="mb-3" style="width: 350px;">
                    <label for="created_at" class="form-label w-100 d-flex justify-content-start">Created at</label>
                    <input type="date" class="form-control" id="created_at" value = "{{ Auth::user()->created_at->format('Y-m-d') }}" readonly>
                </div>
                <div class="mb-3" style="width: 350px;">
                    <label for="updated_at" class="form-label w-100 d-flex justify-content-start">Updated at</label>
                    <input type="date" class="form-control" id="updated_at" value = "{{ Auth::user()->updated_at->format('Y-m-d') }}" readonly>
                </div>
            </form>

            <div class = "p-3" style="max-height: 400px; overflow-y: scroll;">
                @foreach (Auth::user()->posts as $index => $post)
